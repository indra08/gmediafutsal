...
tabel_data = $("#tabel_data").DataTable({
    ajax:{
        url:base_url+"pendaftar/Cheers/ajax_data_pendaftar",
        type:"GET"
    },
    responsive:true,
    processing:true,
...
function verif(id) {
    bootbox.confirm({ 
        ...
        callback: function(result) { 
            if (result) {
                $.ajax({
                    url :base_url+"pendaftar/Cheers/ajax_verifikasi_data/"+id,
                    type : "GET",
                    dataType : "JSON",
                    beforeSend:function() {
                    ...
                    success:function(data) {
                        if(data.status == true) {
                            ...
                            tabel_data.ajax.url(base_url+'pendaftar/Cheers/ajax_data_pendaftar').load();
                        } else {
...
function kirim_invoice(id) {
    $.ajax({
        url :base_url+"pendaftar/Cheers/ajax_kirim_invoice/"+id,
        type : "GET",
        dataType : "JSON",
        beforeSend:function() {
            App.blockUI({boxed:!0,message:msg,textOnly:!0});
